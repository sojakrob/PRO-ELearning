...
namespace ELearning.Business.Managers
{
    public class ManagersContainer
    {
        ...
        private IPermissionsProvider _permissionProvider;

        private Dictionary<Type, object> _managers = new Dictionary<Type, object>();


        public ManagersContainer(IPersistentStorage persistentStorage, IPermissionsProvider permissionsProvider)
        {
        ...
        public T Get<T>() where T : class
        {
            ...
            {
                var ctor = typeof(T).GetConstructor(new[] { typeof(IPersistentStorage), typeof(ManagersContainer), typeof(IPermissionsProvider) });
                if (ctor == null)
